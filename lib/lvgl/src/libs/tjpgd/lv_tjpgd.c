...
#    define TJPGD_WORKBUFF_SIZE (18 * 256)  // Recommended by TJPGD library
...
static lv_result_t decoder_get_area(lv_image_decoder_t *     decoder,
                                    lv_image_decoder_dsc_t * dsc,
...
#    endif
        ...
        if ((lv_strcmp(ext, "jpg") == 0) || (lv_strcmp(ext, "jpeg") == 0)) {
            ...
            JDEC    jd;
            JRESULT rc =
                jd_prepare(&jd, input_func, workb, TJPGD_WORKBUFF_SIZE, &dsc->file);
...
#    if LV_USE_FS_MEMFS
        ...
        if (is_jpg(img_dsc->data, img_dsc->data_size) == true) {
            ...
            lv_fs_make_path_from_buffer(&path, LV_FS_MEMFS_LETTER, img_dsc->data,
                                        img_dsc->data_size);
            lv_fs_res_t res;
...
#    endif
        ...
        if ((lv_strcmp(lv_fs_get_ext(fn), "jpg") == 0) ||
...
static lv_result_t decoder_get_area(lv_image_decoder_t *     decoder,
    ...
    if (decoded_area->y1 == LV_COORD_MIN) {
        ...
        if (decoded == NULL) {
            ...
            JRESULT rc = jd_prepare(jd, input_func, jd->pool_original,
                                    (size_t)TJPGD_WORKBUFF_SIZE, jd->device);
            if (rc)
                return LV_RESULT_INVALID;
        }
