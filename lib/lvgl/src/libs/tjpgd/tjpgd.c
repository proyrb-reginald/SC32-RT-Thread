...
#    if JD_FASTDECODE == 2
#        define HUFF_BIT 6 /* Bit length to apply fast huffman decode */
...
alloc_pool(             /* Pointer to allocated memory block (NULL:no memory available) */
    ...
    if (jd->sz_pool >= ndata) {
        ...
        LV_LOG_WARN("alloc %d B fail", ndata);
    }
    ...
    while (ndata) { /* Process all tables in the segment */
        ...
        for (i = 0; i < 64; i++) { /* Load the table */
            ...
            pb[zi] = (int32_t)((uint32_t)*data++ *
                               Ipsf[zi]); /* Apply scale factor of Arai algorithm to the
                                             de-quantizers */
        }
    }
...
create_huffman_tbl(                      /* 0:OK, !0:Failed */
    ...
    while (ndata) { /* Process all tables in the segment */
        ...
        ph = alloc_pool(
            jd,
            np * sizeof(uint16_t)); /* Allocate a memory block for the code word table */
        if (!ph)
            return JDR_MEM1; /* Err: not enough memory */
...
#    if JD_FASTDECODE == 2
        { /* Create fast huffman decode table */
            ...
            if (cls) {
                tbl_ac =
                    alloc_pool(jd, HUFF_LEN * sizeof(uint16_t)); /* LUT for AC elements */
                if (!tbl_ac)
                    return JDR_MEM1; /* Err: not enough memory */
                ...
                memset(
                    tbl_ac, 0xFF,
                    HUFF_LEN *
                        sizeof(uint16_t)); /* Default value (0xFFFF: may be long code) */
            } else {
                tbl_dc =
                    alloc_pool(jd, HUFF_LEN * sizeof(uint8_t)); /* LUT for AC elements */
                if (!tbl_dc)
                    return JDR_MEM1; /* Err: not enough memory */
            ...
            for (i = b = 0; b < HUFF_BIT; b++) { /* Create LUT */
                for (j = pb[b]; j; j--) {
                    ...
                    if (cls) {
                        td = pd[i++] | ((b + 1) << 8); /* b15..b8: code length, b7..b0:
...
#    if JD_FASTDECODE == 0
    ...
    if ((d & 0xFFD8) != 0xFFD0 || (d & 7) != (rstn & 7)) {
        return JDR_FMT1; /* Err: expected RSTn marker is not detected (may be corrupted
                            data) */
    }
...
#    else
    ...
    if ((marker & 0xFFD8) != 0xFFD0 || (marker & 7) != (rstn & 7)) {
        return JDR_FMT1; /* Err: expected RSTn marker was not detected (may be corrupted
                            data) */
    }
...
JRESULT jd_mcu_load(JDEC * jd /* Pointer to the decompressor object */
    ...
    for (blk = 0; blk < nby + 2; blk++) {      /* Get nby Y blocks and two C blocks */
        ...
        if (cmp && jd->ncomp != 3) { /* Clear C blocks if not exist (monochrome image) */
            ...
            tmp[0] = d * dqf[0] >> 8; /* De-quantize, apply scale factor of Arai algorithm
                                         and descale 8 bits */
...
JRESULT jd_mcu_output(JDEC * jd, /* Pointer to the decompressor object */
    ...
    rx = (x + mx <= jd->width) ?
             mx :
             jd->width - x; /* Output rectangular size (it may be clipped at right/bottom
                               end of image) */
...
jd_prepare(JDEC * jd,                                   /* Blank decompressor object */
    ...
    memset(jd, 0, sizeof(JDEC)); /* Clear decompression object (this might be a problem if
        ...
        switch (marker & 0xFF) {
            ...
            case 0xDA: /* SOS - Start of Scan */
                ...
                for (i = 0; i < jd->ncomp; i++) {
                    ...
                    if (!jd->huffbits[n][0] ||
                        !jd->huffbits[n]
                                     [1]) { /* Check huffman table for this component */
