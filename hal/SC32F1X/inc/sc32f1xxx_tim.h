...
#define IS_TIM_WORKMODE(MODE) (((MODE) == TIM_WorkMode_Timer) || ((MODE) == TIM_WorkMode_Counter))
/**
 * @}
 */
...
#define IS_TIM_EXENX(STATE) (((STATE) == TIM_EXENX_Disable) || ((STATE) == TIM_EXENX_Enable))
/**
 * @}
 */
...
typedef enum {
    TIM_RICPin_Disable =
        (uint16_t)(0x00U << TIM_CON_EXENR_Pos), /*!< No rising input is required for capture   */
    TIM_RICPin_Tn =
        (uint16_t)(0x01U << TIM_CON_EXENR_Pos), /*!< TIM rising input capture pin is Tn   */
} TIM_RICPIN_Typedef;

#define IS_TIM_RICPin(RICPin) (((RICPin) == TIM_RICPin_Disable) || ((RICPin) == TIM_RICPin_Tn))
/**
 * @}
 */
...
typedef enum {
    ...
    TIM_FICPin_TnEx =
        (uint16_t)(TIM_CON_EXENX | TIM_CON_FSEL), /*!< TIM failing input capture pin is TnEx   */
} TIM_FICPIN_Typedef;
...
#    define IS_TIM_ALL_PERIPH(PERIPH)                                                              \
        (((PERIPH) == TIM0) || ((PERIPH) == TIM1) || ((PERIPH) == TIM2) || ((PERIPH) == TIM3) ||   \
         ((PERIPH) == TIM4) || ((PERIPH) == TIM5) || ((PERIPH) == TIM6) || ((PERIPH) == TIM7))
#elif defined(SC32f15xx) || defined(SC32R601)
#    define IS_TIM_ALL_PERIPH(PERIPH)                                                              \
        (((PERIPH) == TIM0) || ((PERIPH) == TIM1) || ((PERIPH) == TIM2) || ((PERIPH) == TIM3))
...
typedef struct {
    ...
    uint16_t
        TIM_WorkMode; /*!< Specifies the Word mode.
                                     This parameter can be a value of @ref TIM_WordMode_Typedef */

    uint16_t
        TIM_CounterMode; /*!< Specifies the Counter mode.
...
typedef struct {
    uint16_t TIM_PWMOutputChannl; /*!<	TIM PWM Channel to be enabled.
                                     ...
                                     value of @ref TIM_PWMChannel_Typedef */
