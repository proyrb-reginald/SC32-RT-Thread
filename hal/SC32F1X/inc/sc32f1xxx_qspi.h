...
typedef enum {
    QSPI_CLKONLY_OFF = (uint32_t)((
        0x00U << TWI_QSPIx_CON_CLKONLY_Pos)), /*!Only send the clock OFF<*/
    QSPI_CLKONLY_ON  = (uint32_t)((
        0x01U << TWI_QSPIx_CON_CLKONLY_Pos)), /*!Only send the clock ON<*/
...
typedef enum {
    QSPI_IT_INTEN = (uint32_t)
        TWI_SPIx_IDE_INTEN, /*!< QSPI Interrupt: QSPI total interruption */
    QSPI_IT_RXNE =
        (uint32_t)TWI_SPIx_IDE_RXNEIE,          /*!< QSPI Interrupt: QSPI RX buffer not
                                                   empty Interrupt */
    ...
    QSPI_IT_QTWIE = (uint32_t)
        TWI_SPIx_IDE_QTWIE, /*!< QSPI Interrupt: SPI total interruption */
} QSPI_IT_TypeDef;

#define IS_QSPI_IT(IT)                                                        \
    (((IT) == QSPI_IT_INTEN) || ((IT) == QSPI_IT_RXNE) ||                     \
     ((IT) == QSPI_IT_TB) || ((IT) == QSPI_IT_RX) || ((IT) == QSPI_IT_RXH) || \
     ((IT) == QSPI_IT_TXH) || ((IT) == QSPI_IT_QTWIE))
/**
 * @}
 */
...
typedef enum {
    QSPI_Flag_RINEIF = (uint32_t)
        TWI_SPIx_STS_RXNEIF, /*!< QSPI Flag: QSPI RX buffer not empty flag */
    QSPI_Flag_TXEIF =
        (uint32_t)TWI_SPIx_STS_TXEIF, /*!< QSPI Flag: TX buffer is empty flag */
    QSPI_Flag_RXFIF = (uint32_t)
        TWI_SPIx_STS_RXFIF, /*!< QSPI Flag: Receiving FIFO overflow flag */
    QSPI_Flag_RXHIF = (uint32_t)
        TWI_SPIx_STS_RXHIF, /*!< QSPI Flag: Receive FIFO over half flag */
    QSPI_Flag_TXHIF = (uint32_t)
        TWI_SPIx_STS_TXHIF, /*!< QSPI Flag: Transmit FIFO under half flag */
    QSPI_Flag_DLUFIF = (uint32_t)TWI_SPIx_STS_DLUFIF,
    QSPI_Flag_WCOL =
        (uint32_t)TWI_SPIx_STS_WCOL, /*!< QSPI Flag: Write conflict flag */
    QSPI_FLAG_QTWIF =
        (uint8_t)TWI_SPIx_STS_QTWIF, /*!< QSPI1 Flag: Interrupt flag */
