...
typedef enum {
    ...
    LCD_FrameFre_B64Hz =
        (uint32_t)(0x00U << DDR_CON_DDRCK_Pos), /*!< The LCD/LED frame frequency of typeB is 64Hz */
    LCD_FrameFre_B128Hz =
        (uint32_t)(0x01U << DDR_CON_DDRCK_Pos), /*!< The LCD/LED frame frequency of typeB is 128Hz*/
    LCD_FrameFre_B256Hz =
        (uint32_t)(0x02U << DDR_CON_DDRCK_Pos), /*!< The LCD/LED frame frequency of typeB is 256Hz*/
    LCD_FrameFre_ACustom =
        (uint32_t)(DDR_CON_TRIMODE |
                   DDR_CON_TYPE), /*!< The LCD/LED frame frequency of typeA is customized*/
    LCD_FrameFre_BCustom =
        (uint32_t)(DDR_CON_TRIMODE), /*!< The LCD/LED frame frequency of typeB is customized*/
...
typedef enum {
    ...
    LCD_Duty_1_4_SEG4_27COM0_3 = (uint32_t)((0x03U << DDR_CFG_DUTY_Pos) |
                                            DDR_CFG_SCS), /*!< LCD/LED display duty cycle: 1/4    */
} LCD_Duty_TypeDef;

#    define IS_LCD_DUTY(DUTY)                                                                      \
        (((DUTY) == LCD_Duty_1_8) || ((DUTY) == LCD_Duty_1_6) || ((DUTY) == LCD_Duty_1_5) ||       \
         ((DUTY) == LCD_Duty_1_4_SEG0_27COM4_7) || ((DUTY) == LCD_Duty_1_4_SEG4_27COM0_3))
...
typedef enum {
    ...
    LCD_Duty_1_4_SEG0_34COM4_7 =
        (uint32_t)(0x03U << DDR_CFG_DUTY_Pos), /*!< LCD/LED display duty cycle: 1/4    */
    LCD_Duty_1_4_SEG4_34COM0_3 = (uint32_t)((0x03U << DDR_CFG_DUTY_Pos) |
                                            DDR_CFG_SCS), /*!< LCD/LED display duty cycle: 1/4    */
} LCD_Duty_TypeDef;

#    define IS_LCD_DUTY(DUTY)                                                                      \
        (((DUTY) == LCD_Duty_1_8) || ((DUTY) == LCD_Duty_1_6) || ((DUTY) == LCD_Duty_1_5) ||       \
         ((DUTY) == LCD_Duty_1_4_SEG0_34COM4_7) || ((DUTY) == LCD_Duty_1_4_SEG4_34COM0_3))
#endif
/**
 * @}
 */
...
typedef enum {
    LCD_Bias_1_4 =
        (uint32_t)(0x00U << DDR_CON_BIAS_Pos), /*!< Display drive bias voltage Settings:1/3 */
    LCD_Bias_1_3 =
        (uint32_t)(0x01U << DDR_CON_BIAS_Pos), /*!< Display drive bias voltage Settings:1/4 */
} LCD_Bias_TypeDef;
...
typedef enum {
    LCD_ResSel_33K =
        (uint8_t)(0x00U
                  << DDR_CON_VOIRSF_Pos), /*!< LCD voltage outlet voltage divider resistor:33K */
    LCD_ResSel_100K =
        (uint8_t)(0x01U
                  << DDR_CON_VOIRSF_Pos), /*!< LCD voltage outlet voltage divider resistor:100K */
    LCD_ResSel_300K =
        (uint8_t)(0x02U
                  << DDR_CON_VOIRSF_Pos), /*!< LCD voltage outlet voltage divider resistor:300K */
    LCD_ResSel_800K =
        (uint8_t)(0x03U
                  << DDR_CON_VOIRSF_Pos), /*!< LCD voltage outlet voltage divider resistor:800K */
} LCD_ResSel_TypeDef;

#define IS_LCD_RESSEL(RES)                                                                         \
    (((RES) == LCD_ResSel_33K) || ((RES) == LCD_ResSel_100K) || ((RES) == LCD_ResSel_300K) ||      \
     ((RES) == LCD_ResSel_800K))
/**
 * @}
 */
...
typedef struct {
    ...
    uint16_t LCD_Voltage; /*!<  Specifies the voltage.
                                                          The VLCD is computed using the following
                             formula: VLCD = VDD*(17+VLCD[3:0]/32) */

    uint32_t LCD_ComPin; /*!< Specifies the Com Pin.
                                                         Each bit represents a Seg channel,Support 8
                            channels.*/
#if defined(SC32f10xx) || defined(SC32f12xx) || defined(SC32R805) || defined(SC32R806)
    uint32_t LCD_SegPin; /*!< Specifies the Seg Pin.
                                                         Each bit represents a Seg channel,Support
                            28 channels.*/
#elif defined(SC32R803)
