...
/**
 * @brief  Initializes the peripheral CMP register according to the parameters specified in
 * CMP_InitStruct
 * @param  CMPx[out]:
 *                SC32f10xx Selection range(CMP)
 *                SC32R803 Selection range(CMP)
...
void CMP_Init(CMP_TypeDef * CMPx, CMP_InitTypeDef * CMP_InitStruct) {
    ...
    if (CMPx == CMP_0 || CMPx == CMP_1 || CMPx == CMP_2) {
        ...
        if (CMPx == CMP_0) {
            ...
            /* Set CMPIS bits and CMPIP bit according to CMP_Sub0_Positive value */
            /* Set CMPIM bits according to CMP_TriggerMode value */
            tmpreg |= (uint32_t)(CMP_InitStruct->CMP_Negative | CMP_InitStruct->CMP_TriggerMode);
            /* Write to CMPx CMP_CFG */
            CMP->CMP_CFG1 = tmpreg;
        } else if (CMPx == CMP_2) {
            /*---------------------------- CMPx CMP_CFG Configuration ------------------------*/
 ...
 *                  SC32R803 Selection
 * range(CMP_Positive_CMP0-CMP_Positive_CMP1,CMP_Positive_1_5V,CMP_Positive_OP) SC32f12xx Selection
 * range(CMP_Positive_CMP0-CMP_Positive_CMP3,CMP_Positive_1_5V,CMP_Positive_OP) SC32f15xx Selection
 ...
 *                  - CMP_Positive_1_5V:Select CMPP as the CMP input port,CMPP is 1.5V reference
 * voltage
 ...
 *               - CMP_CMPSTA_Low:The non-inverting input is at a lower voltage than the inverting
 * input
 *               - CMP_CMPSTA_High: The non-inverting input is at a higher voltage than the
 * inverting input
 *               - CMP_CMP0STA_Low: The non-inverting input is at a lower voltage than the inverting
 * input
 *               - CMP_CMP0STA_High: The non-inverting input is at a higher voltage than the
 * inverting input
 *               - CMP_CMP1STA_Low	: The non-inverting input is at a lower voltage than the
 * inverting input
 *               - CMP_CMP1STA_High: The non-inverting input is at a higher voltage than the
 * inverting input
 *               - CMP_CMP2STA_Low: The non-inverting input is at a lower voltage than the inverting
 * input
 *               - CMP_CMP2STA_High: The non-inverting input is at a higher voltage than the
 * inverting input
 *               - CMP_CMP3STA_Low: CMP3 positive terminal voltage is less than the negative
 * terminal voltage
 *               - CMP_CMP3STA_High:The positive terminal voltage of CMP3 is greater than the
