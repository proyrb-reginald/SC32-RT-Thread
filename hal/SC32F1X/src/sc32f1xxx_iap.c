...
/**
 * @brief  Copy the code data of the function IAP_Program_InRam from ROM to RAM so that the function
 * can be executed in RAM.
 * @param  None
 * @retval None
 */
 ...
 * @param  Type[in]: The type of data to be written, which can be byte (IAP_BYTE), half-word
 * (IAP_HALFWORD), or word (IAP_WORD).
 ...
 * @param  Address[out]: The target address, which is the starting address where the data will be
 * written.
 ...
 *                  - IAP_HALFWORD : Represents a half-word (typically 16 bits or 2 bytes) of data.
 *                  - IAP_WORD : Represents a word (typically 32 bits or 4 bytes) of data.
 * @retval Uint16_t, which typically represents the status or result of the operation (e.g., the
...
boolType IAP_ProgramWord(uint32_t Address, uint32_t Data) {
    ...
    if (((RCC->RCC_CFG0 & (uint32_t)RCC_CFG0_SYSCLKSW) == RESET) ||
        (((RCC->RCC_CFG0 & (uint32_t)RCC_CFG0_SYSCLKSW) != RESET) &&
         ((RCC->RCC_CFG0 & (uint32_t)RCC_CFG0_SYSCLKSEL) == (0x2UL << RCC_CFG0_SYSCLKSEL_Pos)))) {
        /* Check if software lock is open (IAP write operations are permitted) */
        if (IapWriteFlag == IAP_ENABLE) {
            *(__IO uint32_t *)Address = Data;
        }
