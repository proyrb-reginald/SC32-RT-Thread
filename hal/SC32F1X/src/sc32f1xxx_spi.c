...
void SPI_Init(SPI_TypeDef * SPIx, SPI_InitTypeDef * SPI_InitStruct) {
    ...
    tmpreg &= (uint32_t)~(SPI_CON_MSTR | SPI_CON_SPMD | SPI_CON_DORD | SPI_CON_CPHA | SPI_CON_CPOL | SPI_CON_SPR);
    ...
    tmpreg |= (uint32_t)(SPI_InitStruct->SPI_Mode | SPI_InitStruct->SPI_DataSize | SPI_InitStruct->SPI_FirstBit |
                         SPI_InitStruct->SPI_CPHA | SPI_InitStruct->SPI_CPOL | SPI_InitStruct->SPI_Prescaler);
    ...
    if (SPIx == SPI0) {
        ...
        tmpreg = SPIx->SPI_CON;
        /* Clear MSTR, SPMD, DORD, CPOL, SPR bits */
        tmpreg &= (uint32_t)~(SPI0_CON_MSTR | SPI0_CON_SPMD | SPI0_CON_DORD | SPI0_CON_CPHA | SPI0_CON_CPOL | SPI0_CON_SPR);
        /* Configure SPIx: mode, data size, first transmitted bit,clock predivision , CPOL and CPHA
         */
    ...
    if (SPIx == SPI1) {
        ...
        /* Clear MSTR, SPMD, DORD, CPOL, SPR bits */
        tmpreg &=
            (uint32_t)~(SPI1_CON_MSTR | SPI1_CON_SPMD | SPI1_CON_DORD | SPI1_CON_CPHA | SPI1_CON_CPOL | SPI1_TWI1_CON_QTWCK);
        /* Configure SPIx: mode, data size, first transmitted bit,clock predivision , CPOL and CPHA
         */
        /* Set MSTR bits to SPI0_Mode value */
    ...
    tmpreg = SPIx->SPI_CON;
    /* Clear MSTR, SPMD, DORD, CPOL, SPR bits */
    tmpreg &= (uint32_t)~(TWI_QSPIx_CON_SMSTR | TWI_QSPIx_CON_DWIDTH | TWI_QSPIx_CON_DORD | TWI_QSPIx_CON_CPHA |
                          TWI_QSPIx_CON_CPOL | TWI_QSPIx_CON_QTWCK | TWI_QSPIx_CON_MODE);
    /* Configure SPIx: mode, data size, first transmitted bit,clock predivision , CPOL and CPHA */
    /* Set MSTR bits to SPI0_Mode value */
    /* Set SPMD bit according to SPI1_DataSize value */
    /* Set DORD bit according to SPI1_FirstBit value */
